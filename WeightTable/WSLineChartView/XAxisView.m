...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.yValueArray && self.yValueArray.count > 0) {
        ...
        for (NSInteger i = 0; i < self.yValueArray.count; i++) {
            ...
            if (i == self.yValueArray.count-1) {
                ...
                CGContextSetFillColorWithColor(context, aColor.CGColor);//填充颜色
                CGContextAddArc(context, startPoint.x, startPoint.y, 4, 0, 2*M_PI, 0); //添加一个圆
                ...
                if (!_isShowLabel) {
                    ...
                    CGRect strRect = CGRectMake(startPoint.x-strSize.width/2,startPoint.y-strSize.height-3,strSize.width,strSize.height);
                    if (i == 0) {
                        self.firstStrFrame = strRect;
    ...
    if(self.isLongPress)
    {
        ...
        if(nowPoint >= 0 && nowPoint < [self.yValueArray count]) {
            ...
            // 交界点
            CGRect myOval = {selectPoint.x-4, selectPoint.y-4, 8, 8};
...
-(NSString*)removeZeroInFloatString:(NSString*)floatString {
    NSString *outNumber = [NSString stringWithFormat:@"%.1lf", floatString.floatValue];
    return outNumber;
}
